...
This example has two containers, each defining a single endpoint. The endpoint is an accessible port. Each endpoint has a name and port. The server running inside the container is listening on this port. See the following attributes that you can set on the endpoint:
