...
* A command only has one action. Still, you can use `composite` commands to execute several commands either sequentially or in parallel.

. Command group
+
You can assign a given command to one or more groups that represents the nature of the command.  The support groups are: `build`, `run`, `test` and `debug`. For each of the groups, define one default command in each group by specifying the `isDefault` value.
+
[source,yaml]
----
schemaVersion: 2.0.0
