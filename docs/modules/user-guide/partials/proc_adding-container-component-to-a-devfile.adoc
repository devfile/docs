...
+
Use the `container` component to augment the image with additional resources and information to allow for integrating the tooling provided by the image with the application that consumes the devfile.
...
+
Use the `command` attribute of the `container` type to modify the `entrypoint` command of the container created from the image.
