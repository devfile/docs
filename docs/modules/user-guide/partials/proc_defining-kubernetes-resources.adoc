...
[role="_abstract"]
Reference Kubernetes or OpenShift resource lists in devfiles to describe complex deployments. This them makes them part of the workspace.
...
Kubernetes Ingresses are ignored, but OpenShift routes are supported. A workspace created from a {prod-short} using any other object types fails to start.
...
The preceding component references a file that is relative to the location of the {prod-short} . Therefore, this {prod-short}  is only loadable by a factory. You supply the location of your {prod-short} to the factory, and then your {prod-short} determines the location of the referenced Kubernetes resource list.
