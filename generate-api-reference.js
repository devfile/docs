...
fs.readdir('./docs/modules/spec/attachments/jsonschemas/', (err, files) => {
    if (err) throw err;
    files.forEach((version) => {
        var glob = require('glob-fs')();
        var schemas = glob.readdirSync('./docs/modules/spec/attachments/jsonschemas/' + version + '/devfile*.json');
        if (! schemas.empty) {
            var schemaPath = schemas[0]
            var apiReferencePath = './docs/modules/spec/attachments/api-reference/' + version + '/'
            ...
            .then((result) => {
                try {
                    fs.mkdirSync(`./docs/modules/spec/examples/api-reference/${version}/`, { recursive: true })    
                } catch(e) { console.error(e)}
                fs.writeFileSync(
                    `./docs/modules/spec/examples/api-reference/${version}/body.html`,
                    `
